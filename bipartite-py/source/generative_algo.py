...
def selectBooksForFirstReader(gamma_1, simulationParams, **kw):
    '''
    ...
    numBooks = poisson(expr.psiFunction(gamma_1, simulationParams), (1)) # TODO: verify (not sure what psi SUBSCRIPT GAMMA means here..)
    bookScores = {}
    for j in range(numBooks):
        bookScores[j] = sampleFrom15() # TODO: implement
...
def selectBooksForIthReader(gammas, numBooks, prevBookScoreList, simulationParameters):
    ...
    for j in range(numBooks):
        gammaUSum = sum([gammas[k]*math.exp(-prevBookScoreList[k-1].get(j, 0.0)) for k in range(curReaderI)])
    ...
    numAdditionalBooks = poisson(expr.psiTildeFunction(gammas[curReaderI], sum([gammas[k] for k in range(curReaderI)]), simulationParameters), (1)) # TODO: implement
    for j in range(numBooks, numBooks+numAdditionalBooks):
        bookScores[j] = 0.0
    
    # assign scores
    for bookNo in bookScores:
        bookScores[bookNo] = sampleFrom15() # TODO: implement
    
    return numBooks + numAdditionalBooks, bookScores

def generateBipartiteGraph(simulationParameters, gammas):
    numBooks, firstScores = selectBooksForFirstReader(gammas[0], simulationParameters)
